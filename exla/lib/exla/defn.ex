defmodule EXLA.Defn do
  ...
  defp used_inputs(expr) do
    {_, used_inputs} = Tree.composite(expr, %{}, &used_inputs/2)
    used_inputs |> Map.keys() |> Enum.sort()
  end

  defp used_inputs(%T{data: %Expr{op: :parameter, args: [i], context: :root}} = t, acc),
    do: {t, Map.put(acc, i, true)}

  defp used_inputs(t, acc),
    do: Tree.traverse_args(t, acc, &used_inputs/2)
